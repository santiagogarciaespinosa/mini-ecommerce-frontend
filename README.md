...
Aplicación frontend para una tienda tipo mini-ecommerce. Incluye catálogo, carrito con guardado rápido (snapshot local + POST al backend), checkout simulado y panel de administración responsive (tabla en desktop, tarjetas en móvil).
...
Backend DRF corriendo en http://127.0.0.1:8000 (ver “Backend mínimo”)
...
4) Health check rápido del backend
...
Debe responder con estado 201 (creación correcta del carrito).
...
Si el backend protege por integridad (productos en órdenes), verás advertencias y los ítems no se eliminarán (o se aplicará soft delete según configuración).
...
CORS bloqueado → habilitar http://localhost:5173 en django-cors-headers.
